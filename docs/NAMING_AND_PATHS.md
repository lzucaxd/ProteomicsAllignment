...
**Primary doc set:** **[`../MASTER.md`](../MASTER.md)** (single narrative). This page is the **supplement** on path history and naming rules.

The repo grew in stages: CPTAC preprocessing stayed under **`data/`**, the harmonization benchmark under **`scripts/benchmark/`**, and exploratory analyses used to live under **`data/scripts/`** (now mostly **`archive/data_scripts_legacy/`**). That split is **historical**, not an ideal taxonomy.
...
| **3 — Exploratory** | Subtype DA, v1 plots, one-offs | **`archive/data_scripts_legacy/`** (moved from `data/scripts/`) · **new** scripts → **`scripts/exploratory/`** |
...
| **`data/scripts/`** / **`archive/data_scripts_legacy/`** vs **`scripts/`** | Matrix drivers in **`data/`**; benchmark in **`scripts/`**. Exploratory R/Python moved to **`archive/data_scripts_legacy/`**; **`data/scripts/`** keeps two mapping builders — [`../data/scripts/README.md`](../data/scripts/README.md). |
...
| I need… | Go to |
|---------|--------|
| **Single narrative (start)** | [`../MASTER.md`](../MASTER.md) |
| Sample design → MSstats annotation (CPTAC + CCLE) | [`ANNOTATION_FROM_SAMPLES.md`](ANNOTATION_FROM_SAMPLES.md) |
